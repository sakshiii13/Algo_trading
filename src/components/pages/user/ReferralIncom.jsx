...
const ReferralIncome = () => {
  ...
  const fetchIncomeHistory = async () => {
    ...
    try {
      ...
      console.log("LEVEL INCOME ERROR 👉", error);
  ...
  const totalContributors = referralHistory.reduce(
    (acc, curr) =>
      acc + (curr?.contributingUsers?.length || 0),
    0
  );
...
const StatCard = ({
  ...
  return (
    <div className="bg-white/[0.03] border border-white/[0.06] rounded-[24px] p-6">
      ...
      <p className="text-3xl font-black text-white mt-5 font-mono">
        {value}
      </p>
    </div>
  );
